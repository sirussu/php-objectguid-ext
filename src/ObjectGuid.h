...
class TC_GAME_API ObjectGuid
{
    public:
        ...
        explicit ObjectGuid(std::string guid) : _guid(std::stoull(guid, nullptr)) { }
        ...
        std::string GetRawValueString() const { return std::to_string(_guid); }
...
#else
        bool operator==(ObjectGuid const& right) const { return _guid == right._guid; }
        bool operator!=(ObjectGuid const& right) const { return _guid != right._guid; }
        bool operator<(ObjectGuid const& right) const { return _guid < right._guid; }
        bool operator>(ObjectGuid const& right) const { return _guid > right._guid; }
        bool operator<=(ObjectGuid const& right) const { return _guid <= right._guid; }
